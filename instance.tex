...
\fi
Focusing on the first term above, using repeated conditioning one can see that
$\phi \defeq \normsm{ \EE{ (I-\alpha A_1)^\top (I-\alpha A_1)} }<1$ is sufficient 
 to guarantee that the expectation of the squared norm of this term vanishes as $t\to\infty$.
...
there exists $\alpha_P>0$ so that $(\rhod{P}\ge )$ $\rhos{P}>0$  holds for all
$0<\alpha<\alpha_P$ and
...
\end{restatable}
Note that $\beta_t \ge 1/2$ for $t\ge \log(2)/(\alpha \rhos{P})$.
