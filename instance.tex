...
For the MSE to be bounded, we need the spectral norms to be less than unity, implying the conditions $\rhos{P}>0$ and $\rhod{P}>0$. When $P$ is positive definite, then from \Cref{} we can ensure $\rhos{P}>0$ and $\rhod{P}>0$. However, when $P$ is Hurwitz we cannot guarantee  $\rhos{P}>0$ and $\rhod{P}>0$.
...
\paragraph{Admissibility in RL problems:} We now state results which show that certain important class of RL problems are \emph{admissible}.
\begin{theorem}
\begin{itemize}[leftmargin=*]
\item For TD(0) in \Cref{tb:tdalgos}, the class of \emph{on-policy} ($\rho_t=1,\forall t\geq 0$) RL problems is admissible with witness $\alpha=\frac{1}{B^2 d}$
\item For TD(0) in \Cref{tb:tdalgos}, the class of RL problems (\emph{on/off-policy}) with bounded features ($\norm{\phi}^2_2=1$) is admissible  with witness $\alpha=\frac{1}{\rho_{\max}}$.
\item For GTD in \Cref{td:algos}, RL problems are admissible with witness $\alpha=frac{1}{B^2d\rho_{\max}}$
\end{itemize}
\end{theorem}
