...
\end{restatable}
Note that $\beta_t \to 1$ as $t\to\infty$. Hence, the lower bound essentially matches the upper bound (expect that $v^2$ appears in \cref{th:rate} and only $\sigma^2_{b_P}$ appears in \cref{th:lb}). In what follows, we discuss in brief the results in \Cref{th:rate,th:lb}. 
