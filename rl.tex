...
\end{definition}
If $\P$ is admissible, it also follows that an asymptotic fast rate of $O(\frac1t)$ is achieved. To see this, we can choose $\alpha_{\P_U}$ to be the universal step-size and then \Cref{th:rate} holds for all the problems in $\P$. We now define three LVE problem classes considered in this paper as follows:
\begin{comment}
\begin{table}
