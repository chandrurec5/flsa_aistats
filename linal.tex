...
Furthermore, $A=B$ implies $A \qfeq B$, though the converse does not hold.
In particular, for a symmetric matrix $A$, $\lmin{A}>0$ is equivalent to $A \qfgeq 0$, where $0$ stands for the all-zero matrix.

\textbf{Part $(i)$:}  We have $\alpha\phi^\top\phi=\frac{1}{B^2d}\phi^\top_t\phi_t<\beta$ for some $\beta\in (0,1)$. Now, from $A_t = \phi_t(\phi_t-\gamma \phi'_t)^\top$ and $A_t+A_t^\top \qfeq 2 A_t$, we get
\begin{align*}
