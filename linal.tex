...
In what follows, for matrices $A, B\in \R^{\dcd}$, we use $A\qfeq B$, $A\qfleq B$ and $A \qfgeq B$ to denote the fact $x^\top (A-B) x = 0$, $x^\top (A-B) x \leq 0 $ and $x^\top (A-B) x \geq 0$, for all $x\in \R^d$. Here $q$ stands for \emph{quadratic forms}, and note that $A\qfeq A^\top$.
...
&\qfeq 2\phi_t(\phi_t-\gamma\phi'_t)^\top -\beta\phi_t(\phi-\gamma\phi'_t)^\top+\beta\gamma\phi'_t(\phi_t-\gamma{\phi'}_t)^\top\\
...
\begin{align*}
(A_t+A_t^\top) -\alpha A_t^\top A_t 
\qfeq 2\phi_t(\phi_t-\gamma\phi'_t)^\top - (\phi-\gamma\phi'_t)(\phi-\gamma\phi'_t)^\top
