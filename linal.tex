...
\begin{align*}
\E[x^\top F_{t,i+1}y|\F_i]=x^\top (I-\alpha A_P)^{t-i} y\,.
\end{align*}
\end{lemma}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proof}
...
as are $x$ and $y$,
\begin{align*}
\EE{x^\top F_{t,i+1} y | \F_{t-1} } &= x^\top \EE{ (I-\alpha A_t) | \F_{t-1} } F_{t-1,i+1} y=x^\top  (I-\alpha A_P)  F_{t-1,i+1} y\,.
\end{align*}
By the tower-rule for conditional expectations and our measurability assumptions,
\begin{align*}
\EE{x^\top F_{t,i+1} y | \F_{t-2} }=x^\top  (I-\alpha A_P)  \EE{F_{t-1,i+1} |\F_{t-2}} y= x^\top (I-\alpha A_P)^2 F_{t-2,i+1} y\,.
\end{align*}
Continuing this way we get
\begin{align*}
\EE{x^\top F_{t,i+1} y | \F_{t-j} }
= x^\top (I-\alpha A_P)^j F_{t-j,i+1} y\,, \quad j=1,2,\dots,t-i\,.
\end{align*}
Specifically, for $j=t-i$ we get
\begin{align*}
\EE{x^\top F_{t,i+1} y | \F_{i} }  = x^\top (I-\alpha A_P)^{t-i} y\,.
\end{align*}
\end{proof}
...
\begin{align*}
\EE{x^\top F_{t,i+1} \zeta_i | \F_{i} }  = x^\top (I-\alpha A_P)^{t-i} \zeta_i\,.
\end{align*}
Using the tower rule,
\begin{align*}
\EE{x^\top F_{t,i+1} \zeta_i | \F_{i-1} }
= x^\top (I-\alpha A_P)^{t-i}\EE{ \zeta_i | \F_{i-1} }= 0\,.
\end{align*}
\end{proof}
...
= I - \alpha (A_P^\top + A_P) + \alpha^2 \EE{ A_t^\top A_t | \F_{t-1} }$.
Since $(b_t,A_t)_t$ is an independent sequence, $\EE{ A_t^\top A_t|\F_{t-1}} = \EE{ A_1^\top A_1 }$.
Now, using the definition of $\rhos{P}$ from \Cref{def:spect}
$\sup_{x:\norm{x}= 1} x^\top S_t x = 1 - \alpha \inf_{x:\norm{x}=1} x^\top (A_P^\top + A_P - \alpha \EE{A_1^\top A_1}) x
= 1-\alpha \rhos{P}$.
Hence,
\begin{align*}
...
Let $P$ be a distribution over $\R^{\dcd}\times\R^d$ as in \Cref{assmp:lsa} and let $U$ be chosen according to \Cref{lm:hur}. Define $\gamma_t\eqdef U^{-1}\theta_t,\,,\gamma_*\eqdef U^{-1}\ts$, then
\begin{align}
...
\thrate*
\begin{proof}
Let $U$ be the symmetric matrix from \label{sec:hurpd}. Define $\gamma\eqdef U^{-1}\theta$ (and similarly quantities $\hat{\gamma},\gamma_*$)and use \Cref{th:pdrate} to bound $\EE{\norm{\hat{\gamma}_t-\gamma_*}^2}$ (note that the variance in the \Cref{th:pdrate} should be appropriately scaled by $\norm{U^{-1}}^2$). The result follows by recovering $\EE{\norm{\thh_t-\ts}^2}\leq \norm{U}^2\norm{U^{-1}}^2 \EE{\norm{\hat{\gamma}_t-\gamma_*}^2}$
\end{proof}
\begin{comment}
...
$\alpha_P = 1/\lambda$ we see that $\rhos{P}=\rhod{P} = \lambda(2-\alpha \lambda)$. \todoc{The claim that for $\lambda_1<\lambda_2$, $\rhos{P} = \lambda_1(2-\alpha \lambda_1)$ was false.}
Furthermore, thanks to $\EE{N_s}=0$,
\begin{align*}
\EE{\eh^2_{t,1}} =
...
\end{align*}
Summing up these two equalities, we get the desired bound. \todoc{Except I don't see the $\alpha\rhos{P}$ showing up.}
...
\section{Proofs for \cref{sec:xxx}}
