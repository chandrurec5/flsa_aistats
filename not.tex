...
The norm of the matrix $A$ is given by $\norm{A}\eqdef \sup_{x\in \C^d:\norm{x}=1} \norm{Ax}$.  We use $\cond{A}=\normsm{A}\normsm{A^{-1}}$ to denote the condition number of matrix $A$. We denote the spectral radius of a matrix $A$ by $\Lambda(A)=\{\max_i \norm{\lambda_i}: \lambda_i ~\text{is an eigenvalue of}~A\}$.
