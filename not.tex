...
We use $Z\sim P$ to denote the fact that $Z$ (which can be a number, or vector, or matrix) is distributed according to probability distribution $P$; $\E$ denotes mathematical expectation. For two sequences $\{a_t\}_{t\geq 0}$ and $\{b_t\}_{t\geq 0}$ of positive real numbers we write $a_t\asymp b_t$ when there exists constants $c_1\geq c_2\geq 0$ such that $c_2 a_t\leq b_t\leq c_1 a_t$. \todoc{Weaker than what we want.
We have that $a_t,b_t$ can also depend on constants. In this case, $c_1$ and $c_2$ are \emph{not} allowed to depend on these constants.}
